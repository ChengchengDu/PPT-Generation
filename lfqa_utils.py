...
###############
# D2S seq2seq model training
###############
class SlideDatasetS2S(Dataset):
    def __init__(
            self, document_cache, training=True
    ):
        self.document_cache = document_cache
        if training:
            shuffle(self.document_cache)

    def __len__(self):
        return len(self.document_cache)

    def make_example(self, idx):
        in_st = self.document_cache[idx][1].lower()
        out_st = self.document_cache[idx][2].lower()
        return (in_st, out_st)
...
def make_qa_s2s_model(model_name="facebook/bart-large", from_file=None, device="cuda:0"):
    # model的forward是在源代码 modeling bart的forward
    tokenizer = AutoTokenizer.from_pretrained(model_name)
...
def train_qa_s2s_epoch(model, dataset, tokenizer, optimizer, scheduler, args, e=0, curriculum=False, logfile=None):
    if logfile:
        init_logger(logfile)
    model.train()
    # make iterator
    ...
    for step, batch_inputs in enumerate(epoch_iterator):
        # loss = model(**batch_inputs).loss  # new version
        # old version
...
def eval_qa_s2s_epoch(model, dataset, tokenizer, args, logfile=None):
    if logfile:
        init_logger(logfile)
    model.eval()
    # make iterator
...
def train_qa_s2s(qa_s2s_model, qa_s2s_tokenizer, s2s_train_dset, s2s_valid_dset, s2s_args, logfile=None):
    ...
    for e in range(s2s_args.num_epochs):
        ...
        for k, v in qa_s2s_model.state_dict().items():
        ...
        if logfile:
            logger.info("Saving model {}".format(s2s_args.model_save_name))
        else:
            print("Saving model {}".format(s2s_args.model_save_name))
        eval_qa_s2s_epoch(qa_s2s_model, s2s_valid_dset, qa_s2s_tokenizer, s2s_args, logfile=logfile)
...
def make_keyword_dense_index(
    ...
    n_batches = math.ceil(passages_dset.num_rows / batch_size)
    for i in range(n_batches):
        passages = [p for p in passages_dset[i * batch_size: (i + 1) * batch_size]["section_title"]]
        reps = embed_passages_for_retrieval(passages, tokenizer, qa_embedder, max_length, device)
        fp[i * batch_size: (i + 1) * batch_size] = reps
        if i % 50 == 0:
            print(i, time() - st_time)
...
def query_mix_dense_index(
    ...
    res_passages2 = [(int(i), (1 - weight) * float(d)) for i, d in zip(I2[0], D2[0])]
