...
def SIFRank_plus(text, SIF, method="average", N=15, position_bias = 3.4):
    """
    :param text_obj:
    ...
    :return:
    """
    text_obj = InputTextObj(text)   # en_model是用于处理词性标注的
...
class ClusData:
    ...
    def __init__(self, clus_data, all_candidates_embed=None):
        ...
        print("clus_data", clus_data)
        print("all_candiembed")
        print(all_candidates_embed)
        self.keyphrases = clus_data
        assert all_candidates_embed is not None
        self.embed = []
        for w in self.keyphrases:
            if w in all_candidates_embed:
                self.embed.append(list(np.array(all_candidates_embed[w].squeeze_(0))))
            else:
...
if __name__ == "__main__":
    ...
    for section_text in data["text"]:
        ...
        # keyphrase是最终筛选出来的关键词元组
        keyphrase, triple_keyphrase, nn_candi_embed_dict = SIFRank_plus(content_string, SIF, N=15)
        nn_keyphrase = []
        # nn_key_embed_dict = {}
        # for key in keyphrase:
        #     if key in nn_candi_embed_dict:
        #         nn_key_embed_dict[key] = nn_candi_embed_dict[key]
        for key in keyphrase:
       ...
       #  create_pptx.add_slide(prs, slide_num, title_text=cur_section, tf_content=pptx_dict)
        ...
        cls_data = ClusData(nn_keyphrase, nn_candi_embed_dict)
        spclus = Clusterer(cls_data, n_cluster=5, method = 'sp-kmeans')
        spclus.fit()
        center_names = []
        clus_centers = spclus.center_ids
        for cluster_id, keyword_idx in clus_centers:
             keyword = cls_data.id2kp[keyword_idx]
             center_names.append(keyword)
        print("center_names")
        print(center_names)
        
        # # keys = cluster_keys(nn_key_embed_dict)
         
        # # print("nn_keyphrase:", nn_keyphrase)
        # # print("triples", triple_keyphrase)
        # print("keys")
        # print(keys)
